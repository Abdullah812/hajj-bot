...
// التأكد من وجود مجلد قا��دة البيانات
...
// استدعاء قائمة المناسك
bot.onText(/\/rituals/, (msg) => {
  const chatId = msg.chat.id;
  rituals.sendRitualsList(bot, chatId);
});

bot.onText(/\/places/, (msg) => {
  const chatId = msg.chat.id;
  places.sendPlacesList(bot, chatId);
});

bot.onText(/\/guide/, (msg) => {
  const chatId = msg.chat.id;
  guide.sendGuideList(bot, chatId);
});

bot.onText(/\/help/, (msg) => {
  const chatId = msg.chat.id;
  help.sendHelpList(bot, chatId);
});
